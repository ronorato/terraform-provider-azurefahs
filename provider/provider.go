...
import (
	...
	"terraform-provider-azurefahs/services/web"
...
func azureProvider(supportLegacyTestSuite bool) terraform.ResourceProvider {
	p := &schema.Provider{
		...
		ResourcesMap: map[string]*schema.Resource{
			"azurefahs_function_host_secret": web.ResourceFunctionAppHostSecret(),
		},
		DataSourcesMap: map[string]*schema.Resource{
			"azurefahs_function_host_secret": web.DataSourceFunctionHostKey(),
		},
	}
...
func providerConfigure(p *schema.Provider) schema.ConfigureFunc {
	return func(d *schema.ResourceData) (interface{}, error) {
		...
		// helpers.DebugLog("azurerm - Builder: %+v", builder)
		config, err := builder.Build()
		// helpers.DebugLog("azurerm - Config: %+v", config)
		if err != nil {
		...
		// helpers.DebugLog("azurerm - clientBuilder: %+v", clientBuilder)
		client, err := clients.Build(p.StopContext(), clientBuilder)
		if err != nil {
			helpers.DebugLog("azurerm - err = %+v", err)
			return nil, err
		}

		// helpers.DebugLog("azurerm - client: %+v", client)
		...
		// helpers.DebugLog("azurerm - skip_credentials_validation = %t", skipCredentialsValidation)
		helpers.DebugLog("azurerm - client.Account: %+v", client.Account)
		helpers.DebugLog("azurerm - client.Resource: %+v", client.Resource)
